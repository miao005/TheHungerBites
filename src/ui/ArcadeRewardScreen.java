...
public class ArcadeRewardScreen {
    ...
    public void draw(Graphics g, int width, int height) {
        ...
        drawRewardBtn(g2d, btn1, "1. Heal 100% HP + Full Mana", new Color(40,  160, 80),  hoveredBtn == 0, width);
        drawRewardBtn(g2d, btn2, "2. +100% Damage Boost",       new Color(60,  80,  200), hoveredBtn == 1, width);
        drawRewardBtn(g2d, btn3, "3. +25 Max Health",           new Color(160, 40,  160), hoveredBtn == 2, width);
        drawRewardBtn(g2d, btn4, "4. +15 Max Mana",             new Color(180, 140, 20),  hoveredBtn == 3, width);

        NavButtons.draw(g2d, width, height);
    }

    private void drawRewardBtn(Graphics2D g2d, Rectangle r, String text, Color bg,
                               boolean hovered, int screenW) {
    ...
    public void mouseMoved(int mx, int my) {
        int prev = hoveredBtn;
        hoveredBtn = -1;
        if      (btn1 != null && btn1.contains(mx, my)) hoveredBtn = 0;
        else if (btn2 != null && btn2.contains(mx, my)) hoveredBtn = 1;
        else if (btn3 != null && btn3.contains(mx, my)) hoveredBtn = 2;
        else if (btn4 != null && btn4.contains(mx, my)) hoveredBtn = 3;
        if (hoveredBtn != prev) gamePanel.repaint();
    }

    private void applyReward(int choice) {
        switch (choice) {
            case 1: player.setHealth(player.getMaxHealth()); player.setCurrentMana(player.getMaxMana()); break;
            case 2: player.increaseDamage(1.0); break;
            case 3: player.setMaxHealth(player.getMaxHealth()+25); player.setHealth(player.getHealth()+25); break;
            case 4: player.setMaxMana(player.getMaxMana()+15); player.setCurrentMana(player.getCurrentMana()+15); break;
        }
    }
